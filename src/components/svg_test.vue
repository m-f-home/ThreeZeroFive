...
export default {
  name: 'svg_test',
  setup() {
    const rightscissor = ref(null)
    const leftscissor = ref(null)
    const data = reactive({
      scissorAnim(el, rot) {
        TweenMax.to(el, 0.25, {
          rotation: rot,
      ...
      startScissors() {
        data.scissorAnim(rightscissor.value, 30)
        data.scissorAnim(leftscissor.value, -30)
      },
    })
    return { ...toRefs(data), rightscissor, leftscissor }
  },
}
</script>
